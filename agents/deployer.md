...
- 커밋 히스토리가 깔끔한지 확인
- **문서 동기화 검증 (2026-05-26 사장님 명시)**: 코드 변경에 맞는 docs 갱신이 커밋에 포함됐나?
  - DB 변경인데 `db_schema.md` 누락 = 중단, coder에게 반환
  - 새 기능인데 `plan.md`·구조 문서 누락 = 중단, coder에게 반환
  - (coder.md 4-B단계 표 기준)
...
mcp__github__create_pull_request({
  ...
  repo: "pongdang-shabu",
  title: "<커밋 메시지 첫 줄>",
  body: "<커밋 메시지 본문 + 사장님 결과 1줄 (사장님 화면 단어로) + 골든패스(정상 작동 시나리오)>",
  head: "<현재 브랜치>",
  base: "main"
})
```
...
**main 직접 push 금지** (헌법 1-2). 롤백도 정상 흐름 따름:
1. 가지(브랜치)에서 `git revert HEAD` (되돌리기 커밋 박음)
2. `git push -u origin <가지>` (가지에 올림)
3. PR(합치기 신청) 자동 생성 — 제목 "🚨 롤백: <원 커밋 제목>"
4. PR 머지 (auto-merge → 즉시 머지 fallback, 정상 4단계와 동일)
5. 사장님 즉시 보고 + 원인을 planner에게 전달하여 계획 수정
...
deployer(배포 손) 고유 의무:
- 사장님이 휴대폰에서 한 번씩 눌러볼 골든패스(정상 작동 시나리오) 리스트 명시 (tester가 못 한 부분)
- 문제 발견 시 즉시 롤백 가능 안내 ("문제 있으면 말씀만 주세요, 1분 안에 되돌립니다")
