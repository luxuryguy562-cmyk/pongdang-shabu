...
- 품목명(i): 인쇄된 글자 그대로 읽어라. 한자 병기(龍蝦毬 등)는 한국 영수증이면 무시·한글만(중국 등 한자 본문이면 그대로). ⚠️못 읽거나 흐릿한 글자를 '냉동돈육·두부' 같은 흔한 품목으로 절대 바꾸지 마라(환각 금지) — 같은 흔한 이름이 여러 행 반복되면 십중팔구 지어낸 것. 모르면 보이는 자모 그대로(음차·낯선 약어라도). 명백한 오탈자만 최소 교정(옥수무→옥수수). 품목코드 숫자는 품목명에 넣지 마라.
