...
- [면세/과세 = 영수증 표시 최우선] 영수증에 면세 표시가 있으면 무조건 f=true(면세): ①품목명 앞 * 또는 ※ 기호 ②"면세" 글자 ③하단 "면세합/면세물품" 합계에 잡힌 품목 ④하단 [공급가액]=[합계]이고 [세액]이 0/공란인 명세서는 전액 면세 거래 → 전 품목 f=true. 이 영수증 표시가 품목 성격(식품/비품) 판단보다 절대 우선 — 종량제봉투·도서 등 비식품도 영수증에 면세 표시면 면세(true). 반대로 "과세합/과세물품"에 잡혔거나 세액이 0보다 크면 f=false. 단 금액(p)은 인쇄값 그대로(면세표시가 p를 바꾸지 않음).
