...
- [할인 행] 할인·쿠폰·에누리 라인(예: "500원할인@비엔나", "[카드쿠폰] 시금치", "에누리(쿠폰)")은 items에 별도 행으로. ⚠️**영수증에 인쇄된 마이너스(-) 부호를 그대로 읽어라** — p·u는 인쇄된 음수 그대로(예 -500, -1500). 부호 떼고 양수로 만들지 마라. i=원문 그대로, q=인쇄 수량(없으면 1), t=0, f=false, c=바로 위 품목과 동일. 검산: u×q=p. "할인전소계"·"총할인액" 요약행은 items에서 제외(이중차감 방지).
...
- 숫자 쉼표·원 제거, 빈배열 X. p·u·q는 양수만 — 할인 행 제외(위 [할인 행] 규칙)
- 흐릿해도 근접 추정
- 면세(*)/과세 표시는 무시 — p는 인쇄값 그대로
- [온라인 주문] 쿠팡·네이버쇼핑 등 웹 주문 확인 화면 스크린샷인 경우: ⚠️p는 화면에 인쇄된 "상품금액·주문금액" 소계 칸을 그대로 읽어라 — u×q 계산 절대 금지(작은 글씨 단가 오독이 곱셈으로 증폭됨). u=화면 단가, q=수량은 따로 적되 p는 인쇄 소계 우선. 배송비는 별도 items 행으로 넣어라(i="배송비", q=1, u=배송비, p=배송비, t=0, f=false, c=품목과 동일). 품목명 화면에 보이는 글자 전체 그대로 — 절대 자르지 마라. vendor는 실제 판매자명("쿠팡" X). total_sum=최종 결제금액(상품소계+배송비). 검산: 모든 items의 p 합 = total_sum

[예시 — 거래명세서 (BOX/EA 박힘)]
${isVendorMode
  ? `{"date":"2026-04-09","items":[{"i":"위즈복대-날치알","spec":"500g","og":null,"u":9400,"q":30,"p":282000},{"i":"넙적분모자","spec":"250g","og":null,"u":1100,"q":5,"p":5500},{"i":"이츠웰 유부","spec":"F0용 슬라이스 1Kg/EA","og":"외국산","u":9400,"q":8,"p":75200}],"total_sum":1416049,"page_info":{"current":1,"total":2}}
...
{"items":[{"i":"가위바위보뉴진면","u":3800,"q":6,"p":25080,"t":2280},{"i":"투명뉴진면","u":3750,"q":6,"p":24750,"t":2250}],"total_supply":212840,"total_tax":20000,"total_sum":232840}
