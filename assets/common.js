...
- 합계행·소계·부가세행·할인전소계·총할인액합계행·이벤트할인요약행·외상행·용기보증금 = items에서 제외 (공급가액/세액 소계는 total_supply/total_tax로만)
...
- p·u·q는 양수만 — 할인 행 제외
- [할인 행] 품목 목록 표 안의 인라인 할인·쿠폰·에누리 라인(예: "500원할인@비엔나", "[카드쿠폰] 시금치", "금토일N개구매시N원할인@상품명")은 items에 별도 행으로. ⚠️인쇄된 마이너스(-) 부호 그대로 — p·u 음수 그대로(예 -500). i=원문, q=인쇄 수량(없으면 1), t=0, f=false, c=바로 위 품목과 동일. ⚠️품목 목록 아래 합계 구역의 "이벤트 할인"·"총할인액"·"총 할인금액"·"할인 합계" 등 할인 요약 줄은 items 제외 — 위 인라인 할인과 같은 금액을 합계 구역에서 한 번 더 표기한 것(이중차감 방지).
...
- p·u·q는 양수만 — 할인 행 제외
- [할인 행] 품목 목록 표 안의 인라인 할인·쿠폰·에누리 라인(예: "500원할인@비엔나", "[카드쿠폰] 시금치", "에누리(쿠폰)", "금토일N개구매시N원할인@상품명")은 items에 별도 행으로. ⚠️인쇄된 마이너스(-) 부호 그대로 — p·u 음수 그대로(예 -500, -1500). i=원문, spec·og=null, q=인쇄 수량(없으면 1), t=0, f=false, c=바로 위 품목과 동일. ⚠️품목 목록 아래 합계 구역의 "이벤트 할인"·"총할인액"·"총 할인금액"·"할인 합계" 등 할인 요약 줄은 items 제외 — 위 인라인 할인과 같은 금액을 합계 구역에서 한 번 더 표기한 것(이중차감 방지).
