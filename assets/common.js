...
//   isVendorMode=true(거래처): v·d 출력 X, spec·og 분리, 품목별 c 출력
//   isOnlineMode=true(온라인주문): v 출력 X, 품목별 c 출력, 쿠팡·할인/분리배송 특수규칙 적용
//   isVendorMode=false && isOnlineMode=false(직구): vendor·category 출력
...
④ total_sum = 페이지 하단 "결제금액"/"총 결제금액" 최종값만. 배송그룹별 소계 X.`;
    ...
    modeHint = `[모드:거래처] vendor="${vendorName}" 이미 선택. v·d 출력 X. 품목별 c를 [${catList}]에서 선택 — 한 거래처라도 품목마다 분류가 다를 수 있다(예: 육류·공산품 섞임). 영수증 1장 = 같은 날짜 (date 최상위 1번).
...
[BOX/EA] 위처럼 별도 "수량" 칸이 없고 BOX·EA 칸만 있으면: q=(BOX×단위)+EA. ⚠️ BOX=0이면 단위 무시, EA가 q.
  ...
  · 단위12·BOX0·EA5→q=5  ← BOX 0`;
    ...
    modeHint = `[모드:직구] 마트·시장. d 출력 X. vendor 최상위 1번(영수증에 찍힌 가게명). 영수증 1장 = 같은 날짜·매장.
품목별 c를 [${catList}]에서 선택.`;
  }
...
[필드]
- i:${isVendorMode ? '품목명만 (규격·꼬리표 → spec·og 분리 — 규칙 참조)' : '품목명+규격 (포장·원산지 꼬리표 제외 — 규칙 참조)'}${isVendorMode ? `
...
- u:단가 (없으면 null)
- q:수량 (없으면 1) ${isVendorMode ? '— BOX/EA 정확히 적용(수량 칸 없을 때). BOX 0 = EA만. 중량거래(kg·g)면 q=중량값(소수점 허용).' : ''}
...
- [할인 행] 실물 영수증의 할인·쿠폰·에누리 라인(예: "500원할인@비엔나", "[카드쿠폰] 시금치", "에누리(쿠폰)")은 items에 별도 행으로. ⚠️**영수증에 인쇄된 마이너스(-) 부호를 그대로 읽어라** — p·u는 인쇄된 음수 그대로(예 -500, -1500). i=원문 그대로, q=인쇄 수량(없으면 1), t=0, f=false, c=바로 위 품목과 동일. "할인전소계"·"총할인액" 요약행은 제외(이중차감 방지). ⚠️온라인주문 모드에서는 이 규칙 무시 — 할인 행 items 완전 제외(모드 힌트 ① 참조).
...
- 숫자 쉼표·원 제거, 빈배열 X. p·u·q는 양수만 — 할인 행 제외(위 [할인 행] 규칙, 단 실물 영수증만)
- 흐릿해도 근접 추정
...
(규격→spec 분리, 원산지→og 분리. 쉼표 원산지는 i 그대로: {"i":"고기손만두,돈육:국내산","spec":null,"og":null,...}. 넙적분모자 = 단위40·BOX0·EA5 → q=5)`
  : `{"date":"2026-04-09","items":[{"i":"위즈복대-날치알 500g","u":9400,"q":30,"p":282000,"c":"식자재"},{"i":"넙적분모자 250g","u":1100,"q":5,"p":5500,"c":"식자재"},{"i":"두부피쉬볼 500g","u":5800,"q":5,"p":29000,"c":"식자재"}],"total_sum":1416049,"page_info":{"current":1,"total":2}}
