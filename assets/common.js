...
- [할인 행] 가격을 깎는 라인은 모두 음수로. 키워드: "N원할인"·"이벤트할인"·"[카드쿠폰]"·"에누리"·"쿠폰"·"행사할인" 등. items에 별도 행으로 추가. i=원문 그대로(예:"[카드쿠폰] 시금치", "에누리(쿠폰)"), u=음수정수, q=인쇄 수량(없으면 1), p=음수정수(깎는 금액), t=0, f=false. c는 바로 위 품목과 동일. 검산: u×q=p (음수끼리 성립). ⚠️깎는 줄은 무조건 음수 — 영수증에 마이너스 표시 없어도 할인·쿠폰·에누리면 음수로. 단 "할인전소계"·"총할인액" 요약행은 items에서 제외(이중차감 방지).
