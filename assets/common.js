...
- og:원산지. 없으면 null
- u:p÷q 반올림 (세금포함 1 C/S 단가). ⚠️ 영수증 공급가 칸 숫자 그대로 X — 반드시 p/q 계산값 사용
- q:수량 — 위 [수량 칸 우선] 적용
...
[예시 — 대명주류 형태]
{"date":"2026-06-09","items":[{"i":"참이슬","spec":"(유)","og":null,"u":44700,"q":2,"p":89400,"t":8127,"f":false,"c":"주류"},{"i":"맑은란","spec":"(유)","og":null,"u":44700,"q":3,"p":134100,"t":12191,"f":false,"c":"주류"},{"i":"탄산가스","spec":null,"og":null,"u":null,"q":1,"p":0,"t":0,"f":true,"c":"주류"}],"deposit_in":191700,"deposit_out":155800,"total_supply":388363,"total_tax":38837,"total_sum":463100}
(참이슬 p=89400, u=89400÷2=44700. 영수증 공급가 칸 81273 그대로 쓰면 ❌. deposit_in=191,700. deposit_out=155,800. total_sum=463,100)`;
}
...
- og:원산지(예 "국내산"·"중국산"·"미국산"). 없으면 null. ⚠️쉼표로 품명에 자연스럽게 섞인 원산지(예 "고기손만두,돈육:국내산")는 i 그대로, og X
- u:단가 (없으면 null)
- q:수량 (없으면 1)
- p:행 [합계/금액] 칸 인쇄값 그대로 정수(세후=실제 낸 돈). 행마다 [공급가·세액·합계] 칸 따로면 [합계] 칸. u×q 계산 X — 1~2원 차이도 인쇄 우선
- t:행 [세액] 칸 값(정수). 세액 칸이 따로 있으면 그 값, 없거나 면세면 0
...
[규칙]
${_rcpCommonRules()}
- i:품목명만. 규격(괄호·용량·치수 예 "1.5kg망"·"100입")→spec, 원산지→og 분리. "박스입수:N"·"입수:N" → 버림(뒤의 "/원산지"·"※주석" 포함). ⚠️쉼표로 품명에 자연스럽게 섞인 원산지(예 "고기손만두,돈육:국내산", "냉동감자튀김,중국산")는 i 그대로, og X. 예) "이츠웰 유부(F0용 슬라이스 1Kg/EA) 박스입수:8/외국산 ※대두(미국산)" → i:"이츠웰 유부", spec:"F0용 슬라이스 1Kg/EA", og:"외국산". 어디까지가 규격인지 애매하면 자르지 말고 i 그대로 + spec=null. 못 읽으면 보이는 대로, 지어내지 마라.
- p·u·q는 양수만 — 할인 행 제외
- [할인 행] 실물 영수증의 할인·쿠폰·에누리 라인(예: "500원할인@비엔나", "[카드쿠폰] 시금치", "에누리(쿠폰)")은 items에 별도 행으로. ⚠️인쇄된 마이너스(-) 부호 그대로 — p·u 음수 그대로(예 -500, -1500). i=원문, spec·og=null, q=인쇄 수량(없으면 1), t=0, f=false, c=바로 위 품목과 동일. "할인전소계"·"총할인액" 요약행은 제외(이중차감 방지).
...
(규격→spec, 원산지→og 분리. 쉼표 원산지는 i 그대로)
