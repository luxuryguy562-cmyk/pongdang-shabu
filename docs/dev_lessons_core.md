...
## #136 디자인 통일감 — 헌법(CLAUDE.md)에 직접 박아야 효력 발생 (빙산 14회)

`agents/designer.md`에만 규칙 박음 → coder 매 작업 누락 → 통일감 14회 반복 훼손.  
**해결**: 반복 훼손 규칙은 `CLAUDE.md §8` 헌법 최상위에 직접 박음. "agent 파일에 있다" = 사실상 없는 것.
