# 개발 교훈 아카이브 (Dev Lessons Archive)

> ⚠️ **아카이브 파일 — 매 세션 전체 필독 불필요.**
> **핵심 16개는 `docs/dev_lessons_core.md` 참조** (2026-06-02 분리).
> 특정 교훈 검색·참고용으로만 활용.

---

## [2026-06-02] #135 자릿수 자동조정 = 전역 규칙 (화면별 하드코딩 금지)

**사장님 직접 지적 (분노)**:
> "미치셧구나. 저 근무시간이 천시간단위면? 솟자리까지? 감안햇어?"
> "이거 왜 아직도 이렇게 크기 감안안하고 막하지? 디자이너랑 다 박지않앗어?"

**잘못**:
- KPI 카드(`att-kpi-cell.wage .att-kpi-val`) `font-size:22px` 고정 + overflow 처리 없음.
- 인건비 평균 데이터(수십만원)는 멀쩡 → 큰 매장(천 시간·억 단위)에서 카드 밖 넘침.
- designer.md 자릿수 표에 **근무시간·인건비(만원)** 항목 자체가 빠짐 → 시뮬레이션 대상 X.

**CTO 1차 실수**: "1안 = CSS 즉시 수정(클램프+overflow)" → 그 카드만 박는 건 또 하드코딩(헌법 8조 위반).

**해결 (2026-06-02 박음)**:
- `agents/designer.md` 7-A: 🚨 전역 원칙 박음 — "모든 KPI 카드·숫자 셀 = 자동조정 기본값. 화면별 박지 말 것"
- 자릿수 표에 **근무시간(h)**, **인건비/지출(만원 표기)** 항목 추가 + 1억 자동 단위 전환 명시
- `agents/tester.md` 시각 검증에 🚨 **큰 값 자동조정 검증 의무** 추가 — 자릿수 최대치 더미 주입 후 `snap.js` 재실행, 자동조정 작동 확인

**교훈**:
1. 새 카드 만들 때마다 박지 말 것 = 공통 클래스에 박고 상속
2. 자릿수 표에 항목 빠지면 시뮬레이션 안 됨 → 표는 살아있는 문서
3. tester = 평균 데이터만 검증 X. 최대 자릿수 더미 주입 후 자동조정 작동 확인 의무
...
> ⚠️ 2026-06-02 갱신: 텍스트(ASCII) 목업은 **HTML 목업 필수**로 교체됨 — `agents/designer.md` 규칙 #10 참조.
...
## #135 자릿수 자동조정 — 전역 원칙 위반 (2026-06-02)

### 사장님 호소
- "미치셧구나. 저 근무시간이 천시간단위면? 솟자리까지? 감안햇어?"
- "이거 왜 아직도 이렇게 크기 감안안하고 막하지? 디자이너랑 다 박지않앗어?"
- "1번은 왜 하드코딩으로 박지? 전체적으로 봐야하는건데"

### 원인
- designer.md 자릿수 표에 근무시간(h)·인건비(만원 단위) 항목 누락
- tester가 평균 데이터로만 검증 → 큰 값(최대 자릿수) 미검증
- coder가 새 카드 추가 시 고정 px 직접 박음 (전역 원칙 인지 X)
- CTO도 "CSS 즉시 수정(그 카드만)" → 전역 공통 클래스 누락 실수 1차 범함

### 교훈
- KPI 카드·숫자 셀 = 공통 클래스 재사용 + `clamp()` 필수. 화면별 고정 px 금지
- 새 카드 추가 시 → 기존 공통 클래스 확인 → 없으면 공통 클래스에 박고 상속
- tester 의무: 자릿수 최대치 더미(9,999h / 1.2억 / 거래처명 12자) 주입 후 검증

### 조치
- `assets/styles.css` 공통 블록 신설: 13개 클래스 `min-width:0; overflow:hidden; tabular-nums`
- 고정 px 13곳 → `clamp()` 일괄 교체 (근무시간 카드 포함)
- `agents/designer.md` 7-A 전역 원칙 + 자릿수 표에 근무시간·인건비 추가
- `agents/tester.md` 큰 값 자동조정 검증 의무 섹션 신설
- `CLAUDE.md §8` 디자인 절대 원칙 추가 (clamp 의무 + 통일감 규칙)
- `docs/dev_lessons_core.md` #135 박음

---

## #136 빙산 14회 — 디자인 통일감 반복 훼손 (2026-06-02)

### 사장님 호소
- "토스스타일 그것도 잇잖아. 디자인 통일감이 매번 죽긴하거든" (빙산 트리거: 매번)
- 디자인 통일감(카드 스타일·색상·숫자 셀) 반복 훼손 — 14회 누적

### 원인
- 디자인 절대 원칙이 `agents/designer.md` 안에만 묻혀있어 coder가 참조 안 함
- 헌법(CLAUDE.md)에 직접 박히지 않아 매 작업 시 누락
- 빙산 13회 → 14회까지 시스템 갱신 미흡

### 교훈
- 반복 훼손 규칙은 `CLAUDE.md`(헌법 최상위)에 직접 박아야 효력 발생
- "agent 파일에 있다" = 사실상 없는 것 (coder가 항상 참조하는 건 헌법뿐)
- 빙산 14회 → 즉각 헌법·에이전트 갱신 (헌법 §6 자동 의무)

### 조치
- `CLAUDE.md §8` 디자인 절대 원칙 신설: 카드/색상/숫자셀/표/모바일 5개 규칙 직접 박음
- `CLAUDE.md §6` 빙산 카운터 13→14회 갱신
- session-start.sh hook 14회로 갱신
- `docs/dev_lessons_core.md` #136 박음
