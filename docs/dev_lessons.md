...
**후속 (2026-06-24, 같은 빙산 재발 — 코드 단일진실화)**: 프롬프트에 "면세합/과세합 최우선" 규칙은 있었으나 **코드가 그 값을 실제로 안 썼음**. 줄별 세액 0이면 AI 품목 f만 보고 ÷11. 그래서 ① 면세 명세서(공급가액=합계, 세액 공란)에 부가세 오부과(삼겹양지 650,484→부가세 59,135), ② 다른 양식 명세서(에이젯 육류 — 하단 "면세 1,025,592" 박스)에서 AI가 냉동 정육을 과세로 오판 → 또 부가세 오부과. **반쪽 구현 = 또 땜빵(교훈 #3 재확인)**. 코드 단일진실화: AI 응답에 `total_free`(면세칸)·`total_taxable`(과세칸) 추가 + `_noVatDoc` = (면세칸≈합계 & 과세칸≈0) 또는 (공급가액≈합계) → 전 품목 면세 강제(AI 품목 f 무시). receipt.js·common.js. 검증 3케이스(육류면세/공산품과세/쿠팡과세) 통과.
