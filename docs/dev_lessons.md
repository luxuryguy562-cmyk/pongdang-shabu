...
## [2026-06-19] #209 머지 충돌 흔적(<<<< ==== >>>>)이 라이브에 박힘 — 검증 누락 (긴급)

**사고**: PR #720 머지 때 `git merge origin/main` 충돌을 receipt.js·index.html만 풀고 **styles.css의 충돌 흔적을 못 보고** 커밋·squash 머지 → 본가 CSS에 `<<<<<<< HEAD` 등 텍스트가 남아 영수증 화면 꾸밈 깨짐. 다음 작업 중 발견해 #721로 복구.

**원인**: ① 충돌 해결 후 **전체 파일 마커 재검색을 안 함**. ② squash 머지 후에도 작업 가지를 계속 써서 같은 충돌 반복(가지·본가 발산).

**철칙 (머지 시 무조건)**:
1. 충돌 해결 후 **반드시** `grep -rn "^<<<<<<<|^=======|^>>>>>>>" assets/ index.html docs/` 로 **전 파일 마커 0 확인** 후에만 커밋.
2. 머지/푸시 전 `node --check` + 가능하면 Playwright 스냅샷.
3. squash 머지 후 가지 계속 쓰면 `git merge origin/main` 먼저 동기화 + 위 재실행.
4. 본가 반영 직후 `git show origin/main:파일 | grep 마커` 로 라이브 무결성 재확인.

**교훈**: "충돌 풀었다"는 본인 진단도 단정 금지(헌법 1-7-A). 마커 전수 grep = 발언 전 검증(헌법 1-7-C).
