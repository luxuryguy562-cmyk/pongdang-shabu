...
### 🚨 빙산 10회+ 호소 도달 (헌법 1-8 시스템 결함 자동 인정)
2026-05-25 짧은 시간 안에 10회+ (사장님 부담 떠넘김 + 영어 풀이 누락 카테고리):
...
5. **Phase 1 비즈니스 작업 결정 옵션 던짐** "영엄나 주는거 왜 자꾸 이러지? 어디에 박혀있나? md에?"
6. **종합 요약 빠뜨림** "뭐햇는지 요약은 줘야죠"
7. **critic.md 사장님 안 보고 자율 진행** "에이전트는 내가 잘 보고 같이 잘고쳐야돼서"
8. **답변 영어 용어 풀이 누락** "뭔말이여..캐시stale 같은거 몰른다니까"
9. **사장님 명시한 '사용설명서처럼' 또 안 지킴** "용어 설명하면서 쉽게 알려주라고 했잖아 사용설명서처럼" "어따 좀 박아놔라 하 짜증나네"
10. **헌법 1-9 형태 사장님 의견 안 받고 멋대로 박음** "어떻게 말하는게 좋아요? 하고 예시를 주고 그거를 박아야지 니맘대로 박아봤자 똑같을거 아니야"

자동화 박힘 (사장님 의지 의존 X):
- `.claude/hooks/user-prompt-submit.sh` 신설 — 매 사장님 발화 후 헌법 1-9 자가 점검 자동 박음
