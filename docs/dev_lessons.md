...
## 19. 토스 디자인 = 격자선 없음, 여백과 색상으로 구분
테이블에 `border`로 격자를 그리면 엑셀 느낌 → 모바일 앱답지 않음.
토스 스타일은: 여백으로 구분, 배경색 차이, 폰트 크기 차이, 컬러 도트.
모바일에서는 카드형 레이아웃이 테이블보다 가독성 높음.
횡스크롤 테이블은 최후의 수단. 카드 안에 세로 나열이 우선.
