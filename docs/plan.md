...
| 71 | 관리자 대시보드 1차+2차 핵심 (#admin) | ✅ 완료 (2026-05-20) | 어플 주인(김은성) 전용 숨김 화면. PIN 게이트(SHA-256 hash 4자리, 3회 실패 60초 잠금) + AI 비용 카드 5개 + 매장별 가입 유형 그룹핑(🏯프랜차이즈/🏢다매장/🏪개인사업자) + 시뮬 슬라이더 + 학습 규칙(`classification_rules`) CRUD + B안 자동 학습 정리(영수증 삭제 시 고아 학습 노트 자동 청소). 진입: `location.hash === '#admin'`. 매장 UI 인증 무관. DB 변경 없음. docs 정정 동반: business_rules #7 두 사람 5열 표 + dev_lessons #96 (김은성=어플 주인 / 이송은=매장 owner 절대 분리). 3차(Chart.js 차트·CSV·기간 필터·매장 활동·DB 사용량) 다음 세션. |
