...
| 2 | 근태 관리 (출퇴근/CAPS) | ✅ 완료 | |
| 3 | 근무계획 (간트차트) | ✅ 완료 | 2026-05-21 근태 `📋 기록`으로 통합. 시간그리드 v2 안 ③. 주단위 일괄 입력 신설. **2026-05-21 후속 fix**: is_off DB 컬럼 ADD(#190), addSchedSheet 🗑 삭제(#191), 휴무 체크박스 제거→공란=자동 휴무(#192) |
| 4 | 마감정산 (현금 vs 장부) | ✅ 완료 | POS AI 인식 유지 |
| 5 | 대시보드 (매출/지출/순이익) | ✅ 완료 | 일별/주별/월별 정산 + 예비비 관리 + **1순위 카드 + 캘린더 시트 + 휴무 표시** (2026-05-15 PR #123) |
| 6 | 급여 집계 | ✅ 완료 | |
...
| 72 | v17 월/주 카드 v6 갈아엎기 — 매출·예상 + 순수익·예상 + 간트 두 줄 + 상위3 가로 + 상세보기 | ✅ 완료 (2026-05-22, PR #212/#213) | **사장님 호소**: "홈화면이 매출/월/주/달력 등 전부 나열돼 난잡함, 토스처럼 큰 카테고리만 두고 들어가는 흐름 원함". **목업 6라운드 (v1~v6)**: v1 메인 단순화 → v2 동적카드 → v3 회귀+네비위로 → v4 사장님 의도 정정 → v5 카드 양식 갈아엎기 → **v6 1차 확정**. **v6 카드 양식**(월/주 통일): ①매출/예상마감매출 좌우 ②순수익/예상마감순수익 좌우(우측 회색 옅게) ③지출 한 줄(간트 위로) ④간트 두 줄(지출 분포/매출 대비) ⑤상위 카테고리 3개 가로(임계⚠ 분홍) ⑥상세 보기 ▾(펼침=전체 카테고리 풀) ⑦전월 동일 대비+코멘트. **함수**: v17RenderMonthCard / v17RenderWeekCards 갈아엎기. **CSS 신설**: .v17-card-v6 + 7개 하위 클래스. **만원 → 원 단위**(헌법 7조). **FK 점검 동반**: 29개 FK + JSON 내부 FK 0건 깨짐 ✅, 잔재 식자재 대분류 2건은 영향 미미로 보존. **Hotfix PR #213**: `.gantt-bar` 클래스 충돌(근태 간트차트의 position:absolute 적용 받음) → `.v6-bar`로 리네이밍. **변경 라인**: 487(+352/-135). **dev_lessons**: #124(메타 패턴), #125(디자인 라운드 누적), #126(CSS 클래스 이름 충돌) |
| 71 | 관리자 대시보드 1차+2차+3차 완료 (#admin) | ✅ 완료 (2026-05-20, 한 세션 마무리) | **김은성(어플 주인) 전용** 숨김 화면. PIN 게이트(SHA-256 hash 4자리 `1260`, 3회 실패 60초 잠금) + 풀스크린 오버레이. **1차**: 메인 카드(이번달 사용액 30px) + 카드 4종(오늘/모델분포6개+/실패율7일/평균응답) + 누적 카드 + 시뮬 슬라이더. **2차**: 학습 규칙(`classification_rules`) CRUD + 안내 박스(▾) + 적용 영수증 수 + 검색 + B안 자동 학습 정리(영수증 삭제 시 고아 학습 노트 자동 청소). **시점 네비**: 일별/월별 토글 + 데이트피커(date/month) + ◀▶ 화살표 + "오늘로" 자동 노출 + 시점 기준 SQL 분기(어제/지난달 비교). **매장 그룹핑**: 가입 유형 대분류·소분류(🏯프랜차이즈/🏢다매장/🏪개인사업자). **3차**: Chart.js 시간별/일별 추이(막대+라인 이중 Y축) + CSV 다운로드(BOM·매장명 매핑) + Supabase DB 사용량 카드(주요 6테이블 행수+추정MB+500MB대비%) + 에러 로그 카드(최근 5건+총 건수). 진입: `location.hash === '#admin'`. 매장 UI 인증 무관. DB 변경 없음. docs 정정 동반: business_rules #7 두 사람 5열 표 + dev_lessons #96/#97/#98. 다음 페이즈 후보: 외부 매장 권유 준비(PIN 강화·RLS 강화·매장 활동·Freemium) / 공과금 미납 알림 / 거래처 차액 추적. |
| 70 | 월 요약 갈아엎기 + 거래처/주문 UX 대공사 | ✅ 완료 (2026-05-15, PR #126~#139) | **A. 월 요약 단일 표**: 매출/지출/카테고리/순수익/예비비/실수익 모두 한 `<table>` (table-layout:fixed + colgroup 4컬럼). 마감예상 별도 블록 → "월말 예상" 컬럼 통합. 카테고리 = 상위 3(금액 큰 순) + "+ 더보기 N개 ▾"(가운데) + 펼치면 "− 접기 ▴". `parent_id` 자식 있는 카테고리는 우측 "+ 상세보기" → 자식 펼침(0원 자식도 표시). 카테고리 비율 = 매출 대비(지출 합계 X). 일별 추이 그래프 카드 통째 제거(주단위 요약과 정보 중복). **B. 거래처/주문 UX**: 거래종료/완전삭제 → 편집 시트 안으로. 거래처 카드 ☰ 드래그 핸들(handle만 활성, long-press X). 주문 표 7컬럼(날짜/품목/단가/수량/금액/메모/›) — 행 탭=편집 시트(시트 안 빨간 🗑). 주문 입력 버튼 데이터 위로 이동. 거래처 순서 localStorage → store_settings.vendor_order DB(다기기 동기화). **C. 성능**: 지출 hub DB 호출 직렬 8회 → Promise.all 1회 (~9배). 카드 순서 settings 캐시 즉시 적용. 주문 저장 후 vendorMonthTotals 즉시 갱신. **D. dev_lessons**: #80~#85 (추측 금지 / 비율 가치 / 시간 컨텍스트 통합 / parent_id 동적 / 입력 폼 1:1 대조 / 표 정렬·너비 규칙). **DB 변경**: `ALTER TABLE store_settings ADD COLUMN vendor_order TEXT` |
