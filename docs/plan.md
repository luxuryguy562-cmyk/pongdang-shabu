...
| 58 | 매출 관리 페이지 1단계 (sales_records 신설) | ✅ 완료 (2026-04-23) | 새 테이블 `sales_records` (store_id/date/payment_method/category_id/amount/source). 사이드메뉴 "💰 매출 관리" 신설. 표 형식 입력 UI 1개 (행 추가/엑셀 붙여넣기/인라인 편집/삭제). 마감정산 저장 시 `syncClosingToSalesRecords`가 자동으로 결제수단별 한 줄씩 source='closing'으로 INSERT. source 컬럼으로 미래 POS/카드사 API 연동 대비. 2단계(카테고리 자동 분류 + 대시보드 연계 + 실제 API)는 추후. category_type='income' 카테고리만 드롭다운 노출. 색상 구분: 회색=closing, 노랑=excel, 흰=manual, 파란줄=변경됨, 초록줄=새 행 |
