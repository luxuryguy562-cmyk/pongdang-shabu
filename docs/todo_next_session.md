...
### ① [자기-버그] 수동 편집한 `source='closing'` 카드, 마감 재저장 시 덮어써짐
...
### ② [자기-버그] 편집 시트에서 날짜 변경 시 UNIQUE 충돌 가능
...
### ③ [자기-버그] 마감정산 sync 실패 시 사용자 알림 없음
...
### ⑫ 거래내역 테이블에 📸 영수증 배지 추가
- 마감정산엔 있고 거래내역엔 빠짐. 일관성 문제.

### ⑬ 빈 매출 관리 페이지 중앙에 [+ 매출 추가] 큰 버튼
- 현재: 페이지 하단 작은 버튼
- 변경: 빈 상태일 때만 중앙에 큰 버튼 노출

### ⑭ 예비비 사용 이력 대시보드에서 바로 조회
- 예비비 잔고 숫자 탭 → 사용 이력 팝업 시트
