...
**사장님 호소**: ① 직원 화면에 근무계획이 없다(스샷: 직원 하단탭 홈/급여뿐). ② 따로 있는 주간 리스트 근무표(`renderEmpSched`/`empSchedCont`)는 이전 직원화면 급조 때 넣은 것 — 사장님 의도 아님, "왜 있는지 모르겠다". ③ 사장이 직원 많은데 근무계획 일일이 찍는 게 큰 부담.
