...
## ✅ 2026-05-22 완료 — v17 정산현황 탭 전면 개편 (mockup 17번 누적)

- Phase 1~7 모두 완료. main 머지 + 사장님 배포 테스트 대기
- 백업 sha: d691685 (롤백 시 git revert)
- 핵심: [월/주차] 토글 + 월 카드 + 주별 카드 + 모달 + 한 줄 달력 + 끼임 박스 자동 + 문어체 코멘트
- 옛 일별 상세비교 피벗 + 옛 월요약 + 옛 주단위요약 모두 폐기
- 헌법 7-2 단서 추가 (정산현황 탭은 원 단위)
- 자세한 내용: work_log.md 2026-05-22

### 사장님 골든패스 (배포 후 검증)
- [ ] 정산현황 탭 진입 → [📅 월 보기] 디폴트
- [ ] 월 카드 보임 (매출/지출/순수익 + 월말예상 + 간트 + 범례 + 전월동일대비 + 문어체)
- [ ] 캘린더 보임 (월 전체)
- [ ] 진입 카드 → 모달 (주별 카드 5개 비교)
- [ ] [📆 주차 보기] 토글 → 진행중 주차 + 한 줄 달력
- [ ] 주차 ◀▶ 이동
- [ ] 1주차 끼임 박스 (달력 위)
- [ ] 캘린더 셀 탭 → 일별 시트
- [ ] 카테고리 필터 시트 동작
...
### Phase 5 잔여 (다음 세션)
- 예비비 calcReserveBalance를 진마감(mydata_transactions) 기반으로 갈아엎기
- 마이데이터 없으면 폴백 (가마감 옛 방식)
