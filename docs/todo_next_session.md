...
> **최종 업데이트**: 2026-05-17 (인건비 갈라치기 + 금고 변동 사유 갈아엎기 후)
> **이전 세션 완료 내역**: `docs/work_log.md` 2026-05-17 항목 4건 참조
> **현재 브랜치**: main (PR #142~#145 머지)

---

## 🔥 사장님 직접 지정 (2026-05-17)

### agents 워크플로우 강화 — 사장님 고민 후 결정
사장님 답변: "이거 다음 투두리스트에 넣어 지금 좀 고밈좀 해봐야겟늠"

**배경**: 이번 세션에서 critic 질문 1개로 끝, advisor/planner/designer/reviewer/tester 거의 생략 → 사장님이 "에이전트 일 안 하는 거 같다" 비판. 헌법 1-7 추측 2회 (special_wages "활발히 사용", "extra_amount=상여금") + 헌법 3-1 아부 (사장님 단어 "현금지출" 그대로 받고 단방향으로 좁힘).

**후보 방향** (사장님이 고민 후 선택):
1. **agents/*.md 파일 자체 강화**:
   - `critic.md`에 "DB/코드 직접 검증 의무" 못박기 (placeholder/필드명/사용 빈도까지)
   - `coder.md`에 "tester 검증 안 거치면 PR 머지 금지" 추가
   - `context_reader.md`에 "사장님 도메인 단어는 docs grep + DB 직접 조회 의무" 추가
2. **자동 호출 명령어 신설**:
   - 사장님이 "풀코스" 또는 "끝까지" 같은 키워드 치면 → context_reader→critic→advisor→planner→designer→reviewer→coder→tester→deployer 강제 순회
   - 사장님이 "빨리" 또는 "skip" 치면 → 소형 모드 (단축)
3. **다음 시도부터 실제 프로세스 돌리기** (파일 변경 X, 실천만):
   - 매 중형/대형 작업에 context_reader 보고서 출력 의무
   - critic 6강제질문 중 라우팅표대로 2~3개 필수
   - 사장님 짚을 때 핵심 인지 ("내가 안 쓴 거 아니냐" → 즉시 DB 재조회)

**진행 조건**: 사장님이 "agents 고치자" 또는 한 가지 옵션 명시 시.
