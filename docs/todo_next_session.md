...
- **1-4 진입 시점에 index.html 분리 작업으로 전환** (사장님 결정 2026-05-25)
  - 사장님: *"단일로 하면 너무 안좋대, 너도 뭐 찾으려면 불편하지 않음? 지금 잡아야되지않을까싶은데?"*
  - 흐름: 분리 작업 끝 → 1-4 헌법 정정 → 헌법 슬림화 1-5부터 재개
...
3. **1-4 단일 파일 SPA 분리 (지금 진행)** — `index.html` 7,500줄 → 모듈별 분리.
   - 사장님 명시 (2026-05-25): *"하나로만 쓰게 하는거여서 비대해진거네... 잘못된 거같아"* / *"지금 잡아야되지 않을까"*
   - CTO 검증: 자동 머지 충돌 매번 발생 (PR #263, #268 둘 다), CTO 검색·토큰 비효율
   - 진행: 계획서 작성 → 사장님 OK → 단계별 분리 → 1-4 헌법 정정
