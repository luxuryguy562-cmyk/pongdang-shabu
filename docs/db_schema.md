...
| **store_lat / store_lng** (double precision, nullable) | 매장 GPS 좌표 (2026-06-17 추가, migration `add_store_geo_columns`). 사장님이 매장 안에서 "📍 현재 위치를 매장으로 등록"(`registerStoreLocation`) 누르면 저장. 출퇴근 위치검증 GPS 2차 보조용 (WiFi IP 실패 시 대체). |
| **geo_radius_m** (integer, default 150) | GPS 출퇴근 허용 반경(미터, 2026-06-17 추가). 직원 위치가 매장 좌표 반경 안이면 출근 허용. 실내 GPS 오차 대비 기본 150m. `checkGeoForAttendance`에서 Haversine 거리 비교. |
| **role_permissions** (jsonb, default '{}') | 직급별 화면(하단 탭) 권한 (2026-06-15 추가, Management API로 ALTER — MCP read-only 우회). `{직급명: [탭키,...]}` 예: `{"점장":["dashboard","attendance","busHub","expHub","more"], "팀장":["dashboard","attendance"]}`. 탭키 = dashboard(홈)/attendance(근태)/busHub(개시·마감)/expHub(지출)/more(더보기). **빈 {} 또는 직급 키 없으면 = 기존 동작(관리자는 전체)**. owner(사장)=항상 전체(설정 무시). staff(알바)=본인 근태 고정. applyPermissionUI에서 currentEmp.role로 조회. |
...
>
> ⚠️ **2026-06-12 추가** (vendors): `kind='mart'` 값 추가 — 마트·시장(농협·탑마트 등 직접 가서 사는 곳). 영수증 OCR이 상호를 제각각 읽어 한 마트가 여러 이름으로 쪼개지던 문제 해결. 마트도 거래처·온라인처럼 **영수증 종류 선택(거래 채널)의 '마트·시장'에서 목록 골라 찍기** → vendor_id 박힘 + 이름 고정. 온라인처럼 취급품목 없이 AI 자율 분류. 영수증 picker는 **자주 쓴 순 정렬**(일회성은 아래로) + '새 마트' 즉석 추가(`addMartInline`, 이름만). **CHECK 제약 없음 = DDL 불필요, 값만 추가.** 기존 정리 = INSERT 마트 5개(논산농협 하나로마트/광성탑마트/에브리식자재마트 반월·만월점/홈플러스 논산점) + receipts 121건 vendor_id 연결·이름 통일(논산농협 9가지→1, 광성 3가지→1). store_id=4ae03341. 롤백 = `UPDATE receipts SET vendor_id=NULL WHERE vendor_id IN (마트 5개 id)` + 원본 vendor 텍스트는 세션 백업본(2026-06-12 대화)으로 복원. 코드: receipt.js(setRcpMode/renderRcpVendorRow/openRcpVendorPicker/pickRcpVendor/addMartInline/저장 isMartMode).
>
...
| **vat_included** (bool, default true, 2026-06-15) — 금액에 부가세 포함 여부. 포함이면 편집에서 부가세=금액÷11 자동 표시 | |
