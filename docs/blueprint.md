...
> 1. 그 날 **영업개시 실시함** → **영업개시 금고** (`daily_opening.actual_total`)
> 2. 영업개시 **안 함** → **전일 마감 금고** (`전일 settlements.actual_total`)
>
> **흐름**: 마감정산 → 영업개시 → 마감정산 (영업개시 미실시 날은 마감→마감 직결)
> **목적**: 금고가 하나의 사슬로 이어져, 영업개시 입력 오류·밤사이 도난이 마감 차액에서 드러남.
> **함수**: `applySettleStartVault(dateStr)` — `loadOpeningAmount`·`loadOpeningForDate` 공용
> 도입 배경: 2026-06-01 영업개시 금고 오입력(563,700)이 마감에 안 잡힘 → 영업개시·마감 데이터 단절 발견. 그전엔 마감이 전일 마감 금고만 이월받아 영업개시를 무시했음.
...
## 🏪 멀티매장 — "내 매장들" 통합 홈 (2026-06-18 설계 확정)

> 사장님 논의로 확정. 본사 홈 + 매장 선택 + 매장 추가를 **하나의 흐름**으로 통합.

### 매장 운영 3계층 (같은 앱이 성장 따라 확장)
```
1단계  매장 1개 (혼자)        → 로그인 시 바로 그 매장 ("내 매장들" 스킵)
2단계  매장 여러 개 (직영)     → "내 매장들" 홈. 퐁당 여럿 / 음식점+카페. 묶는 키 = 같은 사장 계정(owner)
3단계  프랜차이즈 (본사+가맹점) → "내 매장들" 홈 + 가맹점 초대·감독. 묶는 키 = franchise_id
```
- 화면·흐름 동일, 단계 올라갈수록 기능만 얹힘.
- "내 매장들"에 뜨는 매장 = 내가 접근 가능한 매장 = (내가 owner인 store) ∪ (내가 franchise_admin인 franchise의 store들)

### "내 매장들" 홈 화면 (스케치: 이 세션 작업)
- **진입: 로그인 후 관리자(owner)가 매장 2개+면 자동으로 이 화면, 1개면 바로 대시보드** (`completeLogin`→`_routeManagerHome`). 사이드메뉴 항목 아님 — 사이드(더보기)에 넣었다가 유저플로우 이상으로 제거 (2026-06-19).
- 상단: 전체 매출 + 전체 순익
- 매장 카드: 매장별 매출·순익·비중, **적자 매장 빨강**, 순익순 정렬
- 매장 카드 누름 = **그 매장으로 완전 전환** → 영수증·근태·근무계획·마감정산 전부 (통계만 보는 화면 아님)
- 하단: [+ 매장 추가] / (프랜차이즈면 [가맹점 초대])

### 보안 설계 (RLS 안 건드림 — 격리 유지)
- 현재 RLS: JWT `app_metadata.store_id` 도장으로 단일 매장 격리 (emp-login/emp-session 발급, 위조 불가). **그대로 유지.**
- 문제: JWT 도장 1개라 본사/다매장이 여러 매장 못 봄.
- 해결: **새 edge function `my-stores`(service_role)** — 토큰으로 접근 가능 매장 서버 검증(owner OR franchise_admin) → 매장별 매출·순익 집계 반환. RLS 우회하되 권한은 함수가 검증.
- 매장 전환 = 그 매장 store_id로 세션 재발급(emp-session 재사용 + 권한 확인).
- ⚠️ edge function 배포 = 헌법 8-A 🔴 사장님 "실행 승인" 필요.

### 구현 순서
1. `my-stores` 서버 함수 (권한 검증 + 매장별 매출) ✅ 완료
2. 매장 전환 (switch-store) ✅ 완료
3. "내 매장들" 프론트 화면 + 로그인 후 자동 진입(2개+) ✅ 완료
4. 검증: 다른 매장·다른 사장 데이터 안 새는지 ✅ 완료

### ⏳ 순익(전체 순익) — 미구현, 다음 큰 작업 (2026-06-21 설계)
- **요구**: 매장 안 들어가도 "내 매장들"에서 매장별 순익이 **항상** 보여야 함(저장 방식 X — 사장님 "들어가야만 보이면 별로").
- **공식**: `순익 = 매출 − (식자재 + 인건비 + 고정비 + 로열티 + 카드수수료)` (dashboard.js `netProfit = totalRevenue − totalCostFull`, 650~697)
- **난관(서버 재현이 큰 이유)**:
  - 인건비 = 시급+월급+**주휴수당** 3함수 (헌법 0-7 단일 진실, sidemenu.js)
  - 고정비 = 일별 카테고리별 `round(월합/lastDay)×passedDays` (일할)
  - 로열티·카드수수료 = 진행일까지 날짜별 누적 `prorateByDay` (sidemenu.js)
  - 식자재 = vendor_orders + receipts 합
- **7-7 위험**: 서버(my-stores)에서 위 계산을 재현하면 dashboard와 어긋날 수 있어 회계 깨짐. 서버/클라 언어 달라 단일 함수 호출 불가 → **공식 동기화 + 검증 필수**.
- **구현 방법(제대로)**: ① 매출처럼 my-stores가 매장별 순익을 같은 공식으로 계산 ② **검증 안전핀**: 한 매장(논산) 골라 "매장 대시보드 순익 == 내 매장들 순익" 자동 일치 확인, 어긋나면 막음 ③ 인건비 등 복잡 계산은 단계적으로 옮기며 매번 검증.
- **상태**: 매출은 완료(my-stores v2, settle+ups). 순익은 거대+회계위험이라 검증 체계 갖춰 별도 진행.
...
## 🏢 "내 매장들" 통합 홈 (설계 확정 2026-06-18 — 미구현)

> 사장님 + CTO 논의로 방향 확정. 프랜차이즈·직영 다매장을 **하나의 흐름**으로.
> 스케치: 이 세션에서 Playwright로 가짜 데이터 렌더(사장님 OK). 구현은 대형 작업.

### 배경 — 지금 개념이 3개로 쪼개짐
| 기존 | 문제 |
|---|---|
| `본사 홈`(franchiseHomeCont, franchise_admin 전용) | 사장님도 못 봄(권한 숨김) |
| 매장 선택 시트(storeSheet) | 전환만 |
| 매장 추가(회원가입/가맹점 초대) | 직영 2호점 추가 경로 애매 |

→ "여러 매장을 한 사람이 관리"는 같은 일인데 화면이 셋으로 흩어짐.

### 설계 — "본사 홈"을 "내 매장들"로 일반화
- **프랜차이즈 사장 = 직영 다매장 사장 = 같은 화면.** 차이는 "가맹점 초대" 버튼 유무뿐.
- 로그인 → 매장 1개면 **바로 그 매장**(이 홈 스킵) / 2개+면 **"내 매장들" 홈**
- 화면 구성:
  - 상단: **전체 매출 + 전체 순익**(매출만 X — 사장님은 "어디서 남나"를 봄)
  - 중단: **매장별 카드**(매장명·매출·순익·비중, **적자 매장 빨강 + ⚠**, **순익 순** 정렬), 누르면 그 매장 진입(= 매장 선택 겸함)
  - 하단: `+ 매장 추가` / (프랜차이즈면 `가맹점 초대`)
- 정보 깊이: 통합 홈은 **매출·순익·비중만 간결**. 인건비율·식자재율 등 상세는 **매장 진입 후** 그 매장 대시보드에서.
- 기존 `loadFranchiseHome`의 가맹점 순위 → 이 "매장별 카드"로 흡수.

### ⚠️ 구현 전제 — RLS 다매장 확장 필요 (2026-06-18 보안 검토 발견)
- 현재 RLS(store_isolation) = `store_id = auth.jwt() app_metadata.store_id` → **한 사용자 = 한 매장만** 접근(단일 매장 격리는 견고).
- **본사 관리자가 가맹점 여러 매장 조회 시 이 RLS에 막힘**(JWT store_id 1개만 허용 → 가맹점 데이터 0).
- → 통합 홈 구현 전 **RLS에 "본사/다매장 소유자는 소속 매장(franchise_id 기준) 다 보기" 정책 추가** 또는 본사 조회는 service_role(edge function) 경유 필요.

### 2026-06-18 보안(RLS) 검토 결과 (참고)
- ✅ 전 테이블 RLS 켜짐. 단일 매장 격리 견고(JWT app_metadata 기반, 클라이언트 위조 불가).
- ✅ 정책 0개 테이블 10개(emp_sessions·employee_private·persons·otp_codes·signup_tokens·exp_groups/items 등) = 앱이 직접 sb.from 안 함(서버 전용 deny-all, 안 깨짐).
- ⚠️ `franchises` 정책 = `using true`(누구나 읽기) → 초대코드 노출 가능성, 낮은 우선순위 점검 권장.
- ⚠️ 위 "다매장 조회 막힘" — 통합 홈의 핵심 전제.
