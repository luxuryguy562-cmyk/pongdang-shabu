...
1. 환경변수 `echo $SLACK_WEBHOOK_URL` 보이는지 확인
2. Slack 테스트 메시지 전송 → 사장님 폰 알림 확인
3. Sentry / Cloudflare MCP·키 추가 정보 받기 (필요 시)
4. 신호등 알림 포맷 (🔴🟡🟢) 첫 세팅
...
- 같은 채팅(세션)인데 컨텍스트 압축으로 옛 작업물 못 봄 → 처음부터 재작업 → 사장님 시간 낭비
