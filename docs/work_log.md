...
### ⚠️ 이 진단 + 단정 자체가 실패였음 (다음 PR #240으로 보완)
**사장님이 "잔재 없는 거 확실?" 의심 → 에이전트 "확정" 단정 반복** = 헌법 1-7 위반 + 4번 무시. 진짜 원인 (.sheet box-shadow)을 PR #240에서 발견.

---

## [2026-05-22] viewport 끝 그림자 3가지 차단 (PR #240) — 사장님 "잡히지 않는 그림자" 호소
...
### 사장님 직접 지시로 헌법 갱신
- **헌법 1-7-A 신설**: 에이전트 본인 진단 단정 금지 ("확정/0건/없음" 단정어 사용 시 자가 페널티)
- **헌법 1-11 신설**: 사장님 화면 설명 = 경우의 수 전부 검증 의무
  - 0단계: 에이전트 1차 자체 검증 (현행 OK)
  - 1단계: 1차 안 통함 + 사장님 같은 호소 2번 이상 → 자동 발동
  - 2~4단계: 사장님 발화 키워드 추출 → 각 키워드별 가능성 최소 5개 나열 → 모두 시도
  - 5단계: 매 시도 정직 보고 (단정 X)
- **dev_lessons #127 신설** — 본 케이스 + 시스템 결함 + 갱신 내용 종합
...
### 다음 세션 검증 체크리스트
- [ ] context_reader가 사장님 발화 키워드 추출 후 코드 매칭 자동 시도하나?
