...
## [2026-05-14 후속] 차액 확인 카드 헤더 토스화 + 기간 당월 (소형)

**브랜치**: `claude/improve-deadline-ui-2m897`
**규모**: 소형 (UI 헤더 + 기간 1줄 변경)

### 변경
1. `index.html:1198~1228` 헤더 짜침 → 토스 스타일 (40x40 그라디언트 아이콘 배지 + 15px 800 타이틀 + 11px 서브설명 + 우측 청록 칩)
2. `loadDiffTable()` (`index.html:10493~`) 기간 로직: 최근 30일 → 당월(이번달 1일~오늘)
3. 칩 텍스트 `${month}월` 동적 (5월·6월 자동 갱신)
4. 빈 상태 문구: "최근 마감 데이터 없음" → "이번달 마감 데이터 없음"
