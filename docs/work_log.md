...
## [2026-05-21] 방향성 재정렬 — 사장님 직감 = 정답 (헌법 3-1 적용)

### 사장님 호소
> "지금 방향성이 완전히 무너지는 중인것 같은데 그거에대해서 얘기좀 해보자"

### context_reader + critic 보고 (Claude)
4개 시그널 박힘:
1. 🔴 수익화(헌법 2조 1순위) 거의 정지 — 외부 매장 권유 0%, 영수증 AI 100% 달성 후 한 달간 본인 매장 미시 fix만
2. 🔴 plan.md #13 공과금 미납·#14 거래처 차액 = 1년째 멈춤 (DB 다 깔려있음)
3. 🟡 카테고리·합산 시스템 분기 누적 — `loadExpHubData` ↔ `calcExpenseByCategories` 비대칭 사고(#105) + 좀비 거래처/중복 카테고리
4. 🟡 dev_lessons #80~#107 = reactive fix 27개 누적 (사장님 호소 → Claude 즉시 fix → 또 분기 추가)

### Claude 1차 추천
- A(외부 매장 권유 준비) 1순위 = 헌법 2조 1순위 직역
- B(잔재 정리) 2순위 / C(핵심 기능) 3순위

### 사장님 결정 (Claude 추천 반박)
> "본인 매장 베타 미완 → 외부 권유 못 함. A는 후순위."

= **사장님 본인이 만족 안 하는 앱 = 외부 영업 거짓말**. 헌법 2조 2순위(안정성) + 사업 검증 우선.

### 우선순위 재정렬 (최종)
| 순위 | 작업 |
|---|---|
| 1 | **B. 잔재 정리 + 본인 매장 베타 마무리** |
| 2 | **C. 핵심 기능 완성** (공과금 미납 #13 + 거래처 차액 #14) |
| 3 | **A. 수익화 인프라** (B+C 후 사장님 별도 작업과 병렬) |

### "베타 완료 기준" 후보 (사장님이 다음 세션에 구체화)
- ① 카테고리/합산 시스템 잔재 정리 (Claude 추천)
- ② 공과금 미납 알림 완성
- ③ 거래처 차액 추적 완성
- ④ 사장님이 평소 짚어온 다른 호소들

### Claude 자기검토 (헌법 3-1 반아부 거울)
- 헌법 직역(수익화 1순위) 무비판 적용 → 사장님 사업 직관 무시
- "외부 매장 권유 = 살아있는 데모 전제" 빠뜨림
- → **dev_lessons #108 후보**: Claude critic이 헌법 직역에 매몰. 헌법 우선순위는 단순 순번 아닌 컨텍스트 의존. "Claude도 틀릴 수 있음" 자기검열 의무

### 변경된 docs
- `docs/todo_next_session.md` 상단에 🚨 2026-05-21 방향성 재정렬 섹션 박음
- `docs/work_log.md` (이 항목)
- 다음 세션: dev_lessons #108 박을지 사장님 결정
