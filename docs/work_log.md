...
### 작업 내용
1. **대시보드 전면 재설계**:
   - 월 요약 카드: 매출/지출/순수익/예비비/실수익 (누적+마감예상 + **매출대비 비율%**)
   - 일별 정산 테이블: 매일 매출/지출/순이익 + 흑자🟢/적자🔴 표시
   - 일별 카테고리 상세: 식자재/인건비/고정비/로열티 + **매출대비 %** (터치 펼침)
   - 주정산: 주계행 위에 배치 + 접기/펼치기 (현재주 펼침, 과거주 접힘)
   - 주계 카테고리 합계 + **매출대비 %**
   - 월정산 상세: 엑셀 스타일 항목별 테이블 (비율% 포함)
   - 일별 차트: 매출 라인 + 순이익 라인 동시 표시
   - 원형 차트/아코디언 제거 → 테이블 중심으로 전환
2. **예비비 관리 화면** (사이드메뉴 > 현황 > 예비비):
   - 예비비 잔고 카드 (총적립/총사용/현재잔고)
   - 예비비 설정 (비율/고정액/초기잔고)
   - 예비비 사용 등록 (바텀시트)
   - 사용 이력 리스트
3. **예비비 잔고 미니 표시**: 월 요약 카드 하단에 잔고 + 상세보기 → 예비비 화면 이동
