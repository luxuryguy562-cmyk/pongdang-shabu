...
### 다음 작업 (사장님 "싹다 진행" — 순차)
- 거래처 관리 서브탭(파일 업로드·대조&단가) 제거 + 정산 대조(recon)로 통합 — recon과 vendorCompare 기능 중복, 통합 설계 필요.
- 전체 영수증 목록 화면 (목업 ②) — 거래처·직구·카테고리 무관 통합 조회.
- (후속) 신규 카테고리 생성 시 안 쓰인 무지개색 자동 기본값 배정(saveExpCat) — 확장성.
