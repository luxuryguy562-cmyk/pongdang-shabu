...
### 2026-06-30 로그인 보안 강화 + PIN 6자리 통일
- **새 기기 문자 인증(토스식) 도입 → 즉시 철회**: 공용 기기·브라우저에서 매번 문자 인증이 떠 운영상 불편(사장님 판단). trusted_devices 표 생성 후 삭제, emp-login v10→v11(기기확인 제거). 로그인은 전화+PIN(기기 제한 없음)으로 원복.
- **PIN 4자리 → 6자리 통일**: 로그인 키패드 6칸, 가입·직원추가·complete-signup(v8) 모두 6자리. 기존 직원 000000으로 초기화(persons 12 + employee_private 14, 실행 승인 완료). 첫 로그인 후 각자 변경.
- **본인 PIN 변경 기능 신설**: change-pin 엣지함수(v1) — 세션 확인 → 현재 PIN 검증 → 새 PIN 암호화(HMAC-SHA256) 저장 + 레거시 금고 평문 PIN 제거. 내 이름(헤더)→정보창→🔒 비밀번호 변경. 사장·관리자·직원·개인 모드 공통. (이전엔 PIN 변경 기능 자체가 없었음 — 헌법 1-7-C 위반 정정)
- **홈 월요약 입구 버그 수정**: 월상세 입구가 '이대로 가면' 예상 띠 안에만 있어 월말/월초 예상띠 숨김 시 입구 사라짐 → 월요약 카드 전체 탭 진입 + '월 상세 보기 ›' 항상 노출.
- 관련 엣지함수 현황: emp-login v11, complete-signup v8, change-pin v1(신설). 캐시: sidemenu 20260630d, common 20260630c, dashboard 20260630a.
